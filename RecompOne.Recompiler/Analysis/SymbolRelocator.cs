    ...
    public static Dictionary<uint, uint> Plan(List<MipsFunction> funcs, RelocationEntry[] entries, string overlayName)
    {
        ...
        foreach (var func in funcs)
        {
            ...
            foreach (var instr in func.Instructions)
            {
                ...
                if (op == 0x0F)
                {
                    if (rt != 0) { hi[rt] = (uint)instr.ImmU << 16; hiSet[rt] = true; }
                    continue;
                }
                
                bool consumes = op is 0x09 or 0x0D || (op >= 0x20 && op <= 0x2E) || op is 0x32 or 0x3A;
                if (consumes && rs != 0 && hiSet[rs])
                {
                    uint addr = op == 0x0D ? hi[rs] + instr.ImmU : (uint)(hi[rs] + instr.ImmS);
                    int hit = Match(active, addr);
                    if (hit >= 0)
                    {
                ...
                if (Writes(instr, out int dst) && dst != 0) hiSet[dst] = false;
            }
        }
        
        for (int i = 0; i < active.Length; i++) Console.WriteLine($"[Recompiler] relocated {active[i].Label} at {counts[i]} site(s) in {overlayName}");
        
        return sites;
    }
    
    static int Match(RelocationEntry[] entries, uint addr)
    {
        for (int i = 0; i < entries.Length; i++)
            if (addr >= entries[i].FromAddress && addr < entries[i].FromAddress + entries[i].ByteSize) return i;
        return -1;
    }
    
    //an lui value only survives until something else write that register
    static bool Writes(MipsInstruction i, out int dst)
    {
        dst = 0;
        uint op = i.Word >> 26;
        ...
        if (op == 1) { dst = 31; return (uint)i.Rt is 0x10 or 0x11; }
        if (op == 3) { dst = 31; return true; }
        ...
        if (op >= 0x20 && op <= 0x26) { dst = i.Rt; return true; }
        if (op is 0x10 or 0x11 or 0x12 or 0x13) { dst = i.Rt; return i.Rs is 0 or 2; }
        return false;
    }
