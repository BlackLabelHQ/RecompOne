...
public static class FunctionPipeline
{
    ...
    public static void Run(List<MipsFunction> funcs, MipsInstruction[] instrs, FunctionInfo elfInfo, string name, PipelineOptions options)
    {
    ...
    static void DiscoverCalls(List<MipsFunction> funcs, MipsInstruction[] instrs, IEnumerable<FunctionEntry> noTypeSymbols, string name)
    {
    ...
    static void AddConfigFunctions(List<MipsFunction> funcs, Config.FunctionEntry[] entries, MipsInstruction[] instrs, IEnumerable<FunctionEntry> noTypeSymbols, string name)
    {
    ...
    static void Sweep(List<MipsFunction> funcs, MipsInstruction[] instrs, IEnumerable<FunctionEntry> noTypeSymbols, string name)
    {
        ...
        var callees = FunctionDetector.DiscoverCalls(instrs, funcs, noTypeSymbols, name);
        funcs.AddRange(callees);
        Console.WriteLine($"[Recompiler] linear sweep found {swept.Count} function(s) (+{callees.Count} callees) in {name}");
    }
    
    static void ScanPointers(List<MipsFunction> funcs, MipsInstruction[] instrs, IEnumerable<FunctionEntry> noTypeSymbols, string name)
    {
    ...
    static bool ScanEscapes(List<MipsFunction> funcs, MipsInstruction[] instrs, string name)
    {
    ...
    static void AnalyzeJumpTables(List<MipsFunction> funcs, FunctionInfo elfInfo, string name)
    {
        ...
        if (withTables > 0)
            Console.WriteLine($"[Recompiler] {name}: found jump tables in {withTables} function(s), {entries} entries in total");
    }
